...
public interface CartDAO {
	 ...
	 * @param studentId 로그인 유저의 studentId
	 * @return List<Lesson> 좋아한 강좌 리스트
	 * @throws FindException 좋아한 강좌가 없을 경우에 예외 발생
	 */
	public abstract List<Lesson> selectById(int studentId) throws FindException;
	/**
	 * 유저가 프로젝트에 좋아요를 누르면, 마이페이지 - 내가 좋아한 강좌에 프로젝트가 추가된다.
	 * @param cart 좋아요
	 * @throws Addexception
	 */
	public abstract void insert(Cart cart) throws AddException;
	/**
	 * 좋아요를 누른 프로젝트를 취소한다. 
	 * @param cartId 좋아요를 누른 장바구니의 id 
	 * @return Cart 좋아요를 누른 객체 - 학생, 프로젝트 정보 반환
	 * @throws DeleteException 좋아요를 누르지 않은 강좌를 좋아요 취소할 경우 예외 발생
	 */
	public abstract Cart delete(int cartId) throws DeleteException;
